...
import { fetchClusters, fetchClusterTelemetry } from '../../services/api';
  ...
  useEffect(() => {
    const loadClusters = async () => {
      try {
        ...
        // Load analytics for each cluster
        const clustersWithAnalytics = await Promise.all(
          clustersData.map(async (cluster) => {
            try {
              const analytics = await fetchClusterTelemetry(cluster.clusterId);
              return { ...cluster, analytics };
            } catch (error) {
              console.error(`Failed to load analytics for cluster ${cluster.clusterId}:`, error);
              return cluster;
            }
          })
        );
        
        setClusters(clustersWithAnalytics);
      } catch (error) {
